...
export const useCustomerUnifiedOrders = (orders: Order[]) => {
  const unifiedOrders = useMemo(() => {
    ...
    // Each order should be treated as a single unified order
    // No grouping needed since each order is already complete
    return orders.map(order => {
      // Calculate category details - each order represents one unified transaction
