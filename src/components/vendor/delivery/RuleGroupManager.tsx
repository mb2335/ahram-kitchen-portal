...
export function RuleGroupManager({
  ...
  const getAvailableCategories = (currentCategoryId?: string) => {
    ...
    const usedCategories = editingGroup.rules.map(rule => rule.category_id).filter(id => id && id !== currentCategoryId);
      ...
      <Card>
        <CardHeader>
          ...
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">Create rule groups with to define when delivery is available. Customers need to meet at least one category requirement in the active rule group.</p>
            <Alert>
              ...
              <AlertDescription>
                ...
                {hasActiveGroup && <span className="block mt-1 text-sm font-medium">
        ...
        <CardContent className="space-y-4">
          {/* New Group Creation */}
          {!editingGroup && <div className="flex gap-3 items-end p-4 bg-muted rounded-lg">
              <div className="flex-1">
                <Label htmlFor="group-name">Rule Group Name</Label>
                <Input id="group-name" value={newGroupName} onChange={e => setNewGroupName(e.target.value)} placeholder="e.g., Large Order Rules" />
              </div>
          ...
          {/* Rule Group Editor */}
          {editingGroup && <Card className="border-2 border-primary">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <Input value={editingGroup.name} onChange={e => setEditingGroup({
                ...editingGroup,
                name: e.target.value
              ...
              <CardContent className="space-y-4">
                {editingGroup.rules.map((rule, index) => <div key={index} className="space-y-3">
                    ...
                    <div className="flex gap-3 items-end p-3 border rounded-lg">
                      <div className="flex-1">
                        ...
                        <Select value={rule.category_id} onValueChange={value => updateRule(index, 'category_id', value)}>
                          <SelectTrigger>
                            <SelectValue placeholder="Select category" />
                          </SelectTrigger>
                          <SelectContent>
                            {getAvailableCategories(rule.category_id).map(category => <SelectItem key={category.id} value={category.id}>
                                {category.name}
                              </SelectItem>)}
                          </SelectContent>
                        </Select>
                      ...
                      <div className="w-32">
                        ...
                        <Input type="number" min="1" value={rule.minimum_items} onChange={e => updateRule(index, 'minimum_items', parseInt(e.target.value) || 1)} />
                      </div>
                      {editingGroup.rules.length > 1 && <Button variant="outline" size="sm" onClick={() => removeRule(index)}>
          ...
          <div className="space-y-3">
            {ruleGroups.map(group => <div key={group.id} className="flex items-center justify-between p-4 border rounded-lg">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    {group.is_active && <Badge variant="outline" className="text-xs">
                        Current Rule Set
                      </Badge>}
                  </div>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <Switch checked={group.is_active} onCheckedChange={checked => onToggleRuleGroup(group.id, checked)} />
                  </div>
                  <Button variant="outline" size="sm" onClick={() => duplicateGroup(group)} title="Duplicate group">
                    <Copy className="w-4 h-4" />
                  </Button>
                  <Button variant="outline" size="sm" onClick={() => editExistingGroup(group)}>
                    <Edit className="w-4 h-4" />
                  </Button>
                  <Button variant="outline" size="sm" onClick={() => confirmDelete(group.id)}>
                    <Trash2 className="w-4 h-4" />
                  </Button>
                </div>
              </div>)}
            {ruleGroups.length === 0 && !editingGroup && <div className="text-center py-8 text-muted-foreground">
